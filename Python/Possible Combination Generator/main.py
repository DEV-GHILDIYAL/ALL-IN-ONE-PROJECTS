...
⢸⣿⣿⣿⣷⣏⡠⣰⡏⢀⢀⢀⢀⢀⢀⢀⢀⣠⣴⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⢀⢀⢀⢀⢀⢀⢀⠨⣿⣦⣤⣬⣶⣽⣿⣿⣿⣿⡇
'''
note6 = "\n\nThis will take a long time to collect all combination password"
...
if(isNum == 'y'):
    for i in range(len(num)):
        data.append(num[i])
    size = size + len(num);
    numberOfCombination = numberOfCombination + len(num)
if(isUpperChar == 'y'):
    for i in range(len(upperChar)):
        data.append(upperChar[i])
    size = size + len(upperChar);
    numberOfCombination = numberOfCombination + len(upperChar)
if(isLowerChar == 'y'):
    for i in range(len(lowerChar)):
        data.append(lowerChar[i])
    size = size + len(lowerChar);
    numberOfCombination = numberOfCombination + len(lowerChar)
if(isSpecial == 'y'):
    for i in range(len(special)):
        data.append(special[i])
    size = size + len(special);
    numberOfCombination = numberOfCombination + len(special)

n = 0;
fileName = fileName + ".txt"
file = open(fileName, "a")
note5 = "\n\nwe found total "+ str(numberOfCombination ** 4) + " combinations"
delayLine(note5)
delayLine(note6)
note7 = "\n\nloading...."
delayLine(note7, 1)
    ...
    for j in range(size):
        for k in range(size):
            for l in range(size):
                ...
                # time.sleep(0.05)
file.close()
note8 = "Total " + str(n) + " Combinations Found" 
delayLine(note8)